...
class StatAllocationError extends Error {
  constructor () {
    super()
    this.name = 'StatAllocationError'
    this.message = 'Sum total of stats must be 100'
  }
}
...
const paramTotal = (requestObject) => {
  const total = parseInt(requestObject.hp) + parseInt(requestObject.mp) + parseInt(requestObject.str)
  if (total !== 100) {
    console.log(total)
    throw new StatAllocationError()
  }
}
